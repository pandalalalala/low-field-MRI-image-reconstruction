...
%% Least squares method, noise
    ...
    picture_LSM_ns = reshape(recon_image_LSM,pic_size,pic_size);

%% iteration Kaczmarz_su
    [recon_image_IT,error_IT_ns]=Kaczmarz_su(E_M,Sign_n,1,80); % lambda = 1, max iteration is 10
    picture_IT_ns = reshape(recon_image_IT,pic_size,pic_size);

%% TSVD, noise
    [recon_image_TSVD,error_TSVD_ns] = TSVD(E_M,Sign_n);
    picture_TSVD_ns = reshape(recon_image_TSVD,pic_size,pic_size);
